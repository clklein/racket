(module matcher-test mzscheme
  ...
  (define (make-test-mtch b t) (make-mtch b (non-decomp t)))
  
  (define (test)
    (print-struct #t)
    (test-empty 'any 1 (list (make-test-mtch (make-bindings (list (make-bind 'any 1))) 1)))
    (test-empty 'any 'true (list (make-test-mtch (make-bindings (list (make-bind 'any 'true))) 'true)))
    (test-empty 'any "a" (list (make-test-mtch (make-bindings (list (make-bind 'any "a"))) "a")))
    (test-empty 'any '(a b) (list (make-test-mtch (make-bindings (list (make-bind 'any '(a b)))) '(a b))))
    (test-empty 'any #t (list (make-test-mtch (make-bindings (list (make-bind 'any #t))) #t)))
    (test-empty 1 1 (list (make-test-mtch (make-bindings null) 1)))
    (test-empty 1 '() #f)
    ...
    (test-empty 99999999999999999999999999999999999999999999999
                '()
                #f)
    (test-empty 'x 'x (list (make-test-mtch (make-bindings null) 'x)))
    (test-empty 'x '() #f)
    (test-empty 1 2 #f)
    (test-empty "a" "b" #f)
    (test-empty "a" '(x) #f)
    (test-empty "a" '() #f)
    (test-empty "a" "a" (list (make-test-mtch (make-bindings null) "a")))
    (test-empty #s(x 1) #s(x 1) (list (make-test-mtch (make-bindings null) #s(x 1))))
    (test-empty #s(x 1) #s(x 2) #f)
    (test-empty 'number 1 (list (make-test-mtch (make-bindings (list (make-bind 'number 1))) 1)))
    (test-empty 'number 'x #f)
    (test-empty 'number '() #f)
    (test-empty 'natural 1 (list (make-test-mtch (make-bindings (list (make-bind 'natural 1))) 1)))
    (test-empty 'natural 'x #f)
    (test-empty 'natural '() #f)
    (test-empty 'natural -1 #f)
    (test-empty 'natural 1.0 #f)
    (test-empty 'integer -1 (list (make-test-mtch (make-bindings (list (make-bind 'integer -1))) -1)))
    (test-empty 'integer 'x #f)
    (test-empty 'integer '() #f)
    (test-empty 'integer 1.0 #f)
    (test-empty 'real 1.1 (list (make-test-mtch (make-bindings (list (make-bind 'real 1.1))) 1.1)))
    (test-empty 'real 'x #f)
    (test-empty 'real '() #f)
    (test-empty 'real 2+3i #f)
    (test-empty 'string "a" (list (make-test-mtch (make-bindings (list (make-bind 'string "a"))) "a")))
    (test-empty 'string 1 #f)
    (test-empty 'string '() #f)
    (test-empty 'variable 'x (list (make-test-mtch (make-bindings (list (make-bind 'variable 'x))) 'x)))
    (test-empty 'variable 1 #f)
    (test-empty '(variable-except x) 1 #f)
    (test-empty '(variable-except x) 'x #f)
    (test-empty '(variable-except x) 'y (list (make-test-mtch (make-bindings null) 'y)))
    (test-lang 'x 'y (list (make-test-mtch (make-bindings (list (make-bind 'x 'y))) 'y))
               (list (make-nt 'x (list (make-rhs '(variable-except x))))))
    (test-empty '(variable-prefix x:) 'x: (list (make-test-mtch (make-bindings null) 'x:)))
    (test-empty '(variable-prefix x:) 'x:x (list (make-test-mtch (make-bindings null) 'x:x)))
    ...
    (test-empty `hole
                the-hole
                (list (make-test-mtch (make-bindings (list)) the-hole)))
    (test-empty '(in-hole (hole 2) 1)
                '(1 2)
                (list (make-test-mtch (make-bindings (list)) `(1 2))))
    
    (test-empty '(in-hole (name E_1 ((hide-hole hole) hole)) x)
                `(,the-hole x)
                (list (make-test-mtch (make-bindings (list (make-bind 'E_1 (frame (list the-hole) the-hole (list))))) 
                                      `(,the-hole x))))
    (test-empty '(in-hole (name E_1 ((hide-hole hole) hole)) x)
                `(x ,the-hole)
                #f)
    ...
    (test-empty '(name x number) 1 (list (make-test-mtch (make-bindings (list (make-bind 'x 1) (make-bind 'number 1))) 1)))
    (test-empty 'number_x 1 (list (make-test-mtch (make-bindings (list (make-bind 'number_x 1))) 1)))
    (test-empty 'string_y "b" (list (make-test-mtch (make-bindings (list (make-bind 'string_y "b"))) "b")))
    (test-empty 'any_z '(a b) (list (make-test-mtch (make-bindings (list (make-bind 'any_z '(a b)))) '(a b))))
    
    (test-empty '(name x_!_1 number) 1 (list (make-test-mtch (make-bindings (list (make-bind 'number 1))) 1)))
    ...
    (test-empty '(number_!_1 number_!_1) '(1 2) (list (make-test-mtch (make-bindings (list)) '(1 2))))
    (test-empty '(number_!_1 ...) '(1 2) (list (make-test-mtch (make-bindings (list)) '(1 2))))
    (test-empty '(number_!_1 ...) '(1 2 3 4 5) (list (make-test-mtch (make-bindings (list)) '(1 2 3 4 5))))
    (test-empty '(number_!_1 ...) '(1 2 3 1 5) (list (make-test-mtch (make-bindings (list)) '(1 2 3 1 5))))
    (test-empty '((number_!_1 ...) (number_!_1 ...)) 
                '((1 2 3 1 5) (1 2 3 1 5))
                #f)
    (test-empty '((number_!_1 ...) (number_!_1 ...)) 
                '((17 2 3 1 5) (1 2 3 1 5))
                (list (make-test-mtch (make-bindings (list)) '((17 2 3 1 5) (1 2 3 1 5)))))
    ...
    (test-empty '((number_!_1 number_!_1) ... number_!_1 ...)
                '((1 1) (2 3) 1 4)
                (list (make-test-mtch (make-bindings (list)) '((1 1) (2 3) 1 4))))
    ...
    (test-empty '(a) '(a) (list (make-test-mtch (make-bindings null) '(a))))
    (test-empty '(a) '(b) #f)
    (test-empty '(a b) '(a b) (list (make-test-mtch (make-bindings null) '(a b))))
    (test-empty '(a b) '(a c) #f)
    (test-empty '() 1 #f)
    (test-empty '(#f x) '(#f x) (list (make-test-mtch (make-bindings null) '(#f x))))
    (test-empty '(#f (name y any)) '(#f) #f)
    (test-empty '(in-hole (z hole) a) '(z a) (list (make-test-mtch (make-bindings (list)) '(z a))))
    (test-empty '(in-hole (z hole) (in-hole (x hole) a)) 
                '(z (x a))
                (list (make-test-mtch (make-bindings (list)) '(z (x a)))))
    ...
    (test-empty '(in-hole (in-hole (x hole) hole) y)
                '(x y)
                (list (make-test-mtch (make-bindings (list)) '(x y))))
    
    (test-empty '(number number) '(1 1) (list (make-test-mtch (make-bindings (list (make-bind 'number 1))) '(1 1))))
    (test-empty '((name x number) (name x number)) '(1 1) (list (make-test-mtch (make-bindings (list (make-bind 'x 1) (make-bind 'number 1))) '(1 1))))
    ...
    (test-empty '(a ...) '(a) (list (make-test-mtch (make-bindings empty) '(a))))
    (test-empty '(a ...) '(a a) (list (make-test-mtch (make-bindings empty) '(a a))))
    (test-empty '((name x a) ...) '() (list (make-test-mtch (make-bindings (list (make-bind 'x '()))) '())))
    (test-empty '((name x a) ...) '(a) (list (make-test-mtch (make-bindings (list (make-bind 'x '(a)))) '(a))))
    (test-empty '((name x a) ...) '(a a) (list (make-test-mtch (make-bindings (list (make-bind 'x '(a a)))) '(a a))))
    (test-empty '(hole ...) '() (list (make-test-mtch (make-bindings empty) '())))
    
    (test-empty '(b ... a ...) '() (list (make-test-mtch (make-bindings empty) '())))
    (test-empty '(b ... a ...) '(a) (list (make-test-mtch (make-bindings empty) '(a))))
    (test-empty '(b ... a ...) '(b) (list (make-test-mtch (make-bindings empty) '(b))))
    (test-empty '(b ... a ...) '(b a) (list (make-test-mtch (make-bindings empty) '(b a))))
    (test-empty '(b ... a ...) '(b b a a) (list (make-test-mtch (make-bindings empty) '(b b a a))))
    (test-empty '(b ... a ...) '(a a) (list (make-test-mtch (make-bindings empty) '(a a))))
    (test-empty '(b ... a ...) '(b b) (list (make-test-mtch (make-bindings empty) '(b b))))
    
    (test-empty '(a ..._1 a ..._2) 
                '(a) 
                (list (make-test-mtch (make-bindings (list (make-bind '..._1 1) (make-bind '..._2 0))) '(a))
                      (make-test-mtch (make-bindings (list (make-bind '..._1 0) (make-bind '..._2 1))) '(a))))
    (test-empty '(a ..._1 a ..._1) '(a) #f)
    (test-empty '(a ..._1 a ..._1)
                '(a a) 
                (list (make-test-mtch (make-bindings (list (make-bind '..._1 1))) '(a a))))
    
    (test-empty '((a ..._1 a ..._1) ...)
                '((a a a a)) 
                (list (make-test-mtch (make-bindings (list (make-bind '..._1 '(2)))) '((a a a a)))))
    (test-empty '((a ..._!_1 a ..._!_1) ...)
                '((a a a a)) 
                (list (make-test-mtch (make-bindings '()) '((a a a a)))
                      (make-test-mtch (make-bindings '()) '((a a a a)))
                      (make-test-mtch (make-bindings '()) '((a a a a)))
                      (make-test-mtch (make-bindings '()) '((a a a a)))))

    (test-empty '((name x a) ..._!_1 (name y a) ..._!_1) 
                '(a a) 
                (list (make-test-mtch (make-bindings (list (make-bind 'x '()) (make-bind 'y '(a a)))) '(a a))
                      (make-test-mtch (make-bindings (list (make-bind 'x '(a a)) (make-bind 'y '()))) '(a a))))
    ...
    (test-empty '((name x number) ...) '(1 2) (list (make-test-mtch (make-bindings (list (make-bind 'x '(1 2)) (make-bind 'number '(1 2)))) '(1 2))))
    ...
    (test-lang '(n n ...) '((1 1) 1 1) (list (make-test-mtch (make-bindings (list (make-bind 'n '(1 1)))) '((1 1) 1 1)))
    ...
    (test-empty '(hide-hole a) 'a (list (make-test-mtch (make-bindings '()) 'a)))
    (test-empty '(hide-hole a) '(block-in-hole a) #f)
    (test-empty '(in-hole (x (hide-hole hole)) 1) '(x 1) #f)
    (test-empty '(in-hole (x hole) 1) '(x 1) (list (make-test-mtch (make-bindings '()) '(x 1))))
    (test-empty '(in-hole ((hole #f) (hide-hole hole)) junk)
                '(junk junk2)
                #f)
    
    (test-xab 'lsts '() (list (make-test-mtch (make-bindings (list (make-bind 'lsts '()))) '())))
    (test-xab 'lsts '(x) (list (make-test-mtch (make-bindings (list (make-bind 'lsts '(x)))) '(x))))
    (test-xab 'lsts 'x (list (make-test-mtch (make-bindings (list (make-bind 'lsts 'x))) 'x)))
    (test-xab 'lsts #f (list (make-test-mtch (make-bindings (list (make-bind 'lsts #f))) #f)))
    (test-xab 'split-out '1 (list (make-test-mtch (make-bindings (list (make-bind 'split-out 1))) '1)))

    (test-xab 'exp 1 (list (make-test-mtch (make-bindings (list (make-bind 'exp 1))) 1)))
    (test-xab 'exp '(+ 1 2) (list (make-test-mtch (make-bindings (list (make-bind 'exp '(+ 1 2)))) '(+ 1 2))))
    (test-xab '(in-hole ctxt any)
              '1
              (list (make-test-mtch (make-bindings (list (make-bind 'ctxt the-hole) (make-bind 'any 1))) 1)))
    (test-xab '(in-hole ctxt (name x any))
              '1
              (list (make-test-mtch (make-bindings (list (make-bind 'ctxt the-hole) (make-bind 'x 1) (make-bind 'any 1))) 1)))
    (test-xab '(in-hole (name c ctxt) (name x any))
              '(+ 1 2)
              (list (make-test-mtch (make-bindings (list (make-bind 'ctxt the-hole)
                                                         (make-bind 'c the-hole)
                                                         (make-bind 'x '(+ 1 2))
                                                         (make-bind 'any '(+ 1 2))))
                                    '(+ 1 2))
                    (make-test-mtch (make-bindings (list (make-bind 'ctxt (frame '(+) the-hole '(2)))
                                                         (make-bind 'c (frame '(+) the-hole '(2)))
                                                         (make-bind 'x 1)
                                                         (make-bind 'any 1)))
                                    '(+ 1 2))
                    (make-test-mtch (make-bindings (list (make-bind 'ctxt (frame '(+ 1) the-hole '()))
                                                         (make-bind 'c (frame '(+ 1) the-hole '()))
                                                         (make-bind 'x 2)
                                                         (make-bind 'any 2))) 
                                    '(+ 1 2))))
    (test-xab '(in-hole (name c ctxt) (name i (+ number_1 number_2)))
              ...
              (list (make-test-mtch 
                     (make-bindings (list (make-bind 'i '(+ 1 2))
                                          ...
                                          (make-bind 'ctxt (frame '(+) the-hole '((+ 3 4))))
                                          (make-bind 'c (frame '(+) the-hole '((+ 3 4))))))
                     '(+ (+ 1 2) (+ 3 4)))
                    (make-test-mtch (make-bindings (list (make-bind 'i '(+ 3 4)) 
                                                         (make-bind 'number_1 3)
                                                         (make-bind 'number_2 4)
                                                         (make-bind 'ctxt (frame '(+ (+ 1 2)) the-hole '()))
                                                         (make-bind 'c (frame '(+ (+ 1 2)) the-hole '()))))
                               '(+ (+ 1 2) (+ 3 4)))))
    
    (test-empty '(in-hole ((z hole)) (name x any))
                '((z a))
                (list (make-test-mtch (make-bindings (list (make-bind 'x 'a) (make-bind 'any 'a))) '((z a)))))
    (test-empty '(in-hole (name c (z ... hole z ...)) any)
                '(z z)
                (list 
                 (make-test-mtch (make-bindings (list (make-bind 'c (frame '(z) the-hole '())) 
                                                      (make-bind 'any 'z)))
                                 '(z z))
                 (make-test-mtch (make-bindings (list (make-bind 'c (frame '() the-hole '(z)))
                                                      (make-bind 'any 'z)))
                                 '(z z))))
    (test-empty '(in-hole (name c (z ... hole z ...)) any)
                '(z z z)
                (list 
                 (make-test-mtch (make-bindings (list (make-bind 'c (frame '(z z) the-hole '()))
                                                      (make-bind 'any 'z)))
                                 '(z z z))
                 (make-test-mtch (make-bindings (list (make-bind 'c (frame '(z) the-hole '(z)))
                                                      (make-bind 'any 'z)))
                                 '(z z z))
                 (make-test-mtch (make-bindings (list (make-bind 'c (frame '() the-hole '(z z)))
                                                      (make-bind 'any 'z)))
                                 '(z z z))))
    ...
    (test-empty '(a (in-hole (name c1 (b (in-hole (name c2 (c hole)) d) hole)) e))
                '(a (b (c d) e))
                (list 
                 (make-test-mtch (make-bindings (list (make-bind 'c2 (frame '(c) the-hole '()))
                                                      (make-bind 'c1 (frame '(b (c d)) the-hole '()))))
                            '(a (b (c d) e)))))

    (test-empty '(in-hole (in-hole hole hole) a)
                'a
                (list (make-test-mtch (make-bindings (list)) 'a)))
    
    (test-empty '(a (b (in-hole (name c1 (in-hole (name c2 (c hole)) (d hole))) e)))
                '(a (b (c (d e))))
                (list 
                 (make-test-mtch (make-bindings (list (make-bind 'c1 (frame '(c)
                                                                            (frame '(d) the-hole '())
                                                                            '()))
                                                      (make-bind 'c2 (frame '(c) the-hole '()))))
                            '(a (b (c (d e)))))))
    
    (test-empty '((in-hole (name x (hole b)) a)
                  (in-hole (name x (hole b)) a))
                '((a b) (a b))
                (list (make-test-mtch (make-bindings (list (make-bind 'x (frame '() the-hole '(b)))))
                                      '((a b) (a b)))))
    (test-empty '((in-hole (name x (hole b)) a)
                  (name x (hole b)))
                `((a b) (,the-hole b))
    ...
    (test-xab '(in-hole (a-or-hole ...) a) 
              `(,the-hole a)
              (list (make-test-mtch (make-bindings (list (make-bind 'a-or-hole (list the-hole the-hole))))
                                    `(,the-hole a))))
    ...
    (run-test/equal 'plug-hole (plug the-hole 'a) 'a)
    (run-test/equal 'plug-cons-left (plug `(,the-hole b) 'a) '(a b))
    (run-test/equal 'plug-cons-right (plug `(a ,the-hole) 'b) '(a b))
    (run-test/equal 'plug-frame-unpluggable
                    (plug (frame `(a ,the-hole) the-hole `(,the-hole c)) 'b)
                    `(a ,the-hole b ,the-hole c))
    (run-test/equal 'plug-frame-pluggable-frame
                    (plug (frame `(a ,the-hole) the-hole `(,the-hole c))
                          (frame '(x) the-hole '(y)))
                    (frame `(a ,the-hole) (frame '(x) the-hole '(y)) `(,the-hole c)))
    (run-test/equal 'plug-frame-pluggable-cons
                    (plug (frame `(a ,the-hole) the-hole `(,the-hole c))
                          `(,the-hole b))
                    (frame `(a ,the-hole) `(,the-hole b) `(,the-hole c)))
    (run-test/equal 'plug-no-hole
                    (regexp-match?
                     #rx"term has no hole"
    ...
    (run-test/equal 'plug-no-hole
                    (regexp-match?
                     #rx"term has two holes"
                     (with-handlers ([exn:fail:redex? exn-message])
                       (plug `(a (,the-hole ,the-hole)) 'd)
                       ""))
                    #t)
    ...
    (let ([matches (match-pattern 
                    ...
                    '(1 2 3))])
      (run-test 'frame-as-list-setup #t (and matches #t))
      (run-test 'frame-as-list 
                #t
                (and
                 (andmap (λ (mtch)
    ...
    (test-empty `(+ 1 (side-condition any ,(lambda (bindings) #t) #t))
                '(+ 1 b)
                (list (make-test-mtch (make-bindings (list (make-bind 'any 'b))) '(+ 1 b))))
    ...
    (test-empty `(+ 1 (side-condition b ,(lambda (bindings) #t) #t))
                '(+ 1 b)
                (list (make-test-mtch (make-bindings '()) '(+ 1 b))))
    ...
    (test-xab 'exp_1
              '(+ 1 2)
              (list (make-test-mtch (make-bindings (list (make-bind 'exp_1 '(+ 1 2)))) '(+ 1 2))))
    ...
    (test-xab 'nesting-names
              'b
              (list (make-test-mtch (make-bindings (list (make-bind 'nesting-names 'b))) 'b)))
    (test-xab 'nesting-names
              '(a b)
              (list (make-test-mtch (make-bindings (list (make-bind 'nesting-names '(a b)))) '(a b))))
    (test-xab 'nesting-names
              '(a (a b))
              (list (make-test-mtch (make-bindings (list (make-bind 'nesting-names '(a (a b))))) '(a (a b)))))
    ...
    (test-xab 'same-in-nt
              '(x x)
              (list (make-test-mtch (make-bindings (list (make-bind 'same-in-nt '(x x)))) '(x x))))
    (test-xab 'same-in-nt
              '(x y)
              #f)
    ...
    (test-xab '(in-hole (cross forever-list) 1)
              '(1 x x)
              (list (make-test-mtch (make-bindings '()) '(1 x x))))
    
    (test-xab '(in-hole (cross forever-list) 1)
              '(x 1 x)
              (list (make-test-mtch (make-bindings '()) '(x 1 x))))
    
    
    (test-xab '(in-hole (cross simple) g)
              'g
              (list (make-test-mtch (make-bindings (list)) 'g)))
    
    (test-xab 'var '+ #f)
    (test-xab 'var 'anunusedvariable (list (make-test-mtch (make-bindings (list (make-bind 'var 'anunusedvariable))) 'anunusedvariable)))
    (test-xab 'var 'exp (list (make-test-mtch (make-bindings (list (make-bind 'var 'exp))) 'exp)))
    (test-xab 'var 'exp_x (list (make-test-mtch (make-bindings (list (make-bind 'var 'exp_x))) 'exp_x)))
    
    (test-xab 'underscore '(+ 1 2) (list (make-test-mtch (make-bindings (list (make-bind 'underscore '(+ 1 2)))) '(+ 1 2))))
    (test-xab 'underscore '2 (list (make-test-mtch (make-bindings (list (make-bind 'underscore 2))) 2)))
  ...
  (define (run-test/equal name expected actual)
    (set! test-count (+ test-count 1))
